...
au vimrc User OperatorSandwichAddPost BC()
def BD()
const c = g:operator#sandwich#object.cursor
C(c.tail[1])
C(c.head[1])
enddef
au vimrc User OperatorSandwichDeletePost BD()
var lq = []
def BE(a: bool = true)
const c = a ? [] : g:operator#sandwich#object.cursor.inner_head[1 : 2]
if a || lq !=# c
lq = c
au vimrc User OperatorSandwichAddPost ++once BE(false)
if a
feedkeys('S')
else
...
for lr in ['inoremap', 'snoremap']
exe lr "<expr> JJ      vsnip#expandable() ? '<Plug>(vsnip-expand)' : 'JJ'"
exe lr "<expr> <C-l>   vsnip#available(1) ? '<Plug>(vsnip-expand-or-jump)' : '<C-l>'"
...
setpos('.', b)
enddef
au vimrc BufReadPost * BG()
def BH(a: string, ...b: list<string>)
var c = join(b, ' ')
if empty(c)
c = expand('%:e') ==# '' ? '*' : ($'*.{expand('%:e')}')
endif
...
def CA(a: list<number>, c: string = ''): string
var d = ''
var e = ''
var f = ''
for b in a
...
enddef
nn <script> <C-g> <ScriptCmd>CB()<CR><scriptCmd>CC()<CR>
au vimrc BufNewFile,BufReadPost,BufWritePost * CB('BufNewFile')
def CD(a: string)
if a !=# 'q'
if winnr() ==# winnr(a)
return
...
nn q <Nop>
nn Q q
ExeEach h,j,k,l,q nnoremap q{} <ScriptCmd>CD('{}')<CR>
...
edit
enddef
com! -nargs=1 -complete=file MoveFile CE(<f-args>)
...
enddef
au vimrc Syntax * CG()
au vimrc Syntax javascript,vim CH('SpellRare', '\s[=!]=\s')
au vimrc Syntax vim CH('SpellRare', '\<normal!\@!')
def CI()
var a = H()->join('')
...
enddef
au vimrc OptionSet list silent! DC()
au vimrc BufNew,BufReadPost * silent! DC()
set t_Co=256
syntax on
set bg=dark
...
enddef
au vimrc VimEnter * ++nested if !D()|DD()|endif
