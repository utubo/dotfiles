...
au vimrc User CursorMovedDelay BJ()
if has('win32')
def CB(): string
return &ff !=# 'dos' ? $' {&ff}' : ''
enddef
...
enddef
nn <C-g> <ScriptCmd>call <SID>DA()<CR><scriptCmd>call <SID>DB()<CR>
au vimrc BufNewFile,BufReadPost,BufWritePost * DA('BufNewFile')
def DC(a: string = '')
if !!a
if winnr() ==# winnr(a)
return
...
nn g! <Cmd>update<CR><Cmd>source %<CR>
